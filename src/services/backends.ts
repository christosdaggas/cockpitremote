...
async function detectOne(def: BackendDef, unitFiles: string[], processSockets: ListeningSocket[]): Promise<BackendInfo> {
    ...
    if ((def.id === "grd" || def.id === "grd-rdp") && binaryPath) {
        ...
        if (!ok || !/^Overall:/m.test(output)) {
            ...
            supported = false;
            notes.push(_("Could not read GNOME Remote Desktop's status (\"grdctl status\" failed), so endpoint capability is unknown. Refresh to retry."));
        } else {
            const grd = parseGrdStatus(output);
            if (def.id === "grd-rdp") {
                if (!grd.hasRdp) {
                    supported = false;
                    notes.push(_("This GNOME Remote Desktop build provides no RDP backend."));
                } else {
                    detectedPort = resolveGrdRdpPort(grd, processSockets);
                    remoteLoginPort = await detectRemoteLoginPort();
                    notes.push(_("Runs in the logged-in user's GNOME session and uses the standard RDP protocol."));
                    if (detectedPort && detectedPort !== grd.rdpPort)
                        notes.push(format(_("GNOME negotiated port $0 because port $1 is occupied by the system Remote Login service."),
                                          detectedPort, grd.rdpPort));
                    if (remoteLoginPort)
                        notes.push(format(_("GNOME \"Remote Login\" is also enabled on port $0. It opens a headless session that adopts the browser's resolution, so fullscreen fills the window exactly — pick it under Settings if you do not need to see the physical screen."),
                                          remoteLoginPort));
                    notes.push(_("The browser console uses local guacd as the RDP gateway."));
                    if (!grd.rdpEnabled)
                        notes.push(_("RDP is currently disabled. Enable it with GNOME Settings or grdctl."));
                    if (grd.rdpViewOnly)
                        notes.push(_("View-only mode is on, so remote input is ignored."));
                    if (grd.rdpPasswordEmpty)
                        notes.push(_("RDP password authentication is selected but no password is stored."));
                }
            } else if (!grd.hasVnc) {
                supported = false;
                notes.push(_("This GNOME Remote Desktop build provides no VNC backend (recent upstream versions are RDP-only) — the console cannot connect to it."));
            } else {
                detectedPort = grd.vncPort;
                vncScreenShareMode = await getVncScreenShareMode();
                notes.push(_("Runs in the logged-in user's session — service control and settings apply to the user you are logged into Cockpit as."));
                if (vncScreenShareMode === "mirror-primary")
                    notes.push(_("Screen sharing is selected, so VNC records the primary monitor of the logged-in GNOME session. With nobody logged in at a monitor — after a reboot, or when the only session is a headless Remote Login — there is nothing to record, and connections are dropped right after authentication. Pick the virtual monitor under Settings for access that needs no physical screen."));
                else if (vncScreenShareMode === "extend")
                    notes.push(_("A virtual monitor is selected, so VNC creates its own screen for each connection and follows the resolution the browser asks for. It needs no physical monitor and does not show the physical desktop."));
                notes.push(_("Its VNC server listens on all network interfaces and cannot be limited to 127.0.0.1. The console tunnels through Cockpit either way — consider blocking outside access to the VNC port with a firewall rule."));
                if (!grd.vncEnabled)
                    notes.push(_("VNC is currently disabled. Enable it with \"grdctl vnc enable\" as the desktop user, or in GNOME Settings under Remote Desktop."));
                if (grd.vncViewOnly)
                    notes.push(_("View-only mode is on, so remote input is ignored. Allow control with \"grdctl vnc disable-view-only\"."));
                if (grd.vncAuthMethod === "prompt")
                    notes.push(_("Authentication is set to \"prompt\": each connection must be approved on the host's desktop. For unattended access run \"grdctl vnc set-auth-method password\" and \"grdctl vnc set-password\"."));
                else if (grd.vncAuthMethod === "password" && grd.vncPasswordEmpty)
                    notes.push(_("Password authentication is selected but no password is stored — connections will fail until one is set with \"grdctl vnc set-password\"."));
            }
        }
    }
    return {
